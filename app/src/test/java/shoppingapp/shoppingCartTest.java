...
public class shoppingCartTest {
    ...
    public void testAddProductCart() {
        ...
        boolean actual = cart.addItem("HP Envy");

        assertEquals("Test adding item to cart", expected, actual);
    }

    @Test
    public void testRemoveProductCart() {
        boolean expected = true;
        boolean actual = cart.removeItem(digital_2.getName());

        assertEquals("Test removing item from cart", expected, actual);
    }

    @Test
    public void testTotalCartAmount() {
        double expected = 400.5;
        double actual = cart.cartAmount();
        double delta = 0.1;

        assertEquals("Test calculating total amount", expected, actual, delta);
    }
